    ...
    if message.channel.id == UPLOAD_CHANNEL_ID and message.attachments:
        async with aiohttp.ClientSession() as session:
            for attachment in message.attachments:
                ...
                if fname in ["message.txt", "weekly reports.txt"]:
                    timestamp = int(time.time())
                    clean_name = fname.replace(" ", "_")  # Optional: make filename safe
...
async def sendreport(interaction: discord.Interaction):
    ...
    for fname in sorted(os.listdir(FOLDER_NAME)):
        ...
        try:
            ftime = int(fname.split("_")[0])  # Extract timestamp from name
        except ValueError:
            continue  # Skip any files without valid timestamps
