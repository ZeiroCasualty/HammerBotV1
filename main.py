...
# --- Report Upload Logic ---
UPLOAD_CHANNEL_ID = 1298102353358225418
TARGET_CHANNEL_ID = 1298113350814793738
...
async def on_ready():
    ...
    if not scheduler_started:
        trigger = CronTrigger(hour=23, minute=45, day_of_week='mon-fri', timezone="UTC")
        ...
        async with aiohttp.ClientSession() as session:
            for attachment in message.attachments:
                ...
                if fname in ["message.txt", "weekly.txt"]:
                    timestamp = current_time  # Capture timestamp at time of upload
                    clean_name = fname.replace(" ", "_")  # Make filename safe
                    save_path = os.path.join(FOLDER_NAME, f"{timestamp}_{clean_name}")
...
async def sendreport(interaction: discord.Interaction):
    ...
    if interaction.channel.id != TARGET_CHANNEL_ID:
        await interaction.followup.send("⚠️ You can't use this command here.", ephemeral=True)
        return
    ...
    # Collect all recent valid files
    for fname in sorted(os.listdir(FOLDER_NAME)):
        fpath = os.path.join(FOLDER_NAME, fname)
        try:
            ftime = int(fname.split("_")[0])
            actual_name = "_".join(fname.split("_")[1:])  # Get original filename
        except ValueError:
            continue

        if now - ftime <= TIME_LIMIT and actual_name in valid_filenames:
            matching_files.append((ftime, fpath))

    # Sort by upload time, send in order
    ...
    if sent:
        await interaction.response.send_message("✅ Reports sent.")
    else:
