...
async def on_message(message):
    ...
    if message.channel.id == UPLOAD_CHANNEL_ID and message.attachments:
        saved_any = False  # To track if any files were successfully saved
        async with aiohttp.ClientSession() as session:
            for attachment in message.attachments:
                fname = attachment.filename.lower()
                if fname in ["message.txt", "weekly reports.txt"]:
                    timestamp = int(time.time())
                    save_path = os.path.join(FOLDER_NAME, f"{timestamp}.txt")
        ...
        if saved_any:
            await message.channel.send("✅ File(s) uploaded! Use `/sendreport` within 60 seconds.")
